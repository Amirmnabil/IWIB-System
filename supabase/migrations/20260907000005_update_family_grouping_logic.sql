-- Migration: Update family grouping logic in process_endorsement_invoicing RPC
-- Allows linking dependents to parent employee by Staff ID pattern (with "_" or "-") OR by matching Individual ID (member_id_tpa).
...
BEGIN
  ...
  IF p_members_to_insert IS NOT NULL AND jsonb_array_length(p_members_to_insert) > 0 THEN
    ...
    -- Post-process to resolve linked_main_member_id for family requests where the employee was added in the same batch or exists in policy
    -- Rule: Match by principle_id OR Staff ID pattern (with "-" or "_") OR Individual ID (member_id_tpa)
    UPDATE public.policy_members pm
    SET linked_main_member_id = parent.id
    FROM public.policy_members parent
    WHERE pm.policy_id = v_policy_id
      AND pm.linked_main_member_id IS NULL
      AND lower(pm.relation) <> 'employee'
      AND lower(pm.relation) <> 'principal'
      AND parent.policy_id = v_policy_id
      AND (lower(parent.relation) = 'employee' OR lower(parent.relation) = 'principal')
      AND pm.addition_date = parent.addition_date
      AND (
        (pm.principle_id IS NOT NULL AND parent.staff_code = pm.principle_id)
        OR (
          parent.staff_code IS NOT NULL AND pm.staff_code IS NOT NULL AND
          (
            parent.staff_code = regexp_replace(pm.staff_code, '[-_]\d+$', '') OR
            regexp_replace(parent.staff_code, '[-_]\d+$', '') = regexp_replace(pm.staff_code, '[-_]\d+$', '')
          )
        )
        OR (pm.member_id_tpa IS NOT NULL AND pm.member_id_tpa <> '' AND parent.member_id_tpa = pm.member_id_tpa)
      );
  END IF;
