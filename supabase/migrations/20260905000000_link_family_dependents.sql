-- Migration: Link family dependents on endorsement approval
-- Updates process_endorsement_invoicing RPC to resolve linked_main_member_id for spouse/children additions in family requests.
...
BEGIN
  ...
  IF p_members_to_insert IS NOT NULL AND jsonb_array_length(p_members_to_insert) > 0 THEN
    ...
    -- Post-process to resolve linked_main_member_id for family requests where the employee was added in the same batch
    ...
    WHERE pm.policy_id = v_policy_id
      ...
      AND pm.principle_id IS NOT NULL
      AND pm.relation <> 'Employee'
      AND pm.relation <> 'Principal'
      AND parent.policy_id = v_policy_id
      AND parent.staff_code = pm.principle_id
      AND (parent.relation = 'Employee' OR parent.relation = 'Principal')
