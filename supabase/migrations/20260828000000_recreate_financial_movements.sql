-- Migration: recreate_financial_movements and process_endorsement_invoicing RPC
-- Created: 2026-08-28 00:00:00

-- 1. Create reference_list table if not exists (in case it wasn't run)
...
-- 2. Create policy_financial_movements table if not exists
...
-- 3. Create invoice_financial_movements table if not exists
...
-- 4. Enable RLS and add basic select/all policies if not already active
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'policy_financial_movements' AND policyname = 'Enable all access for authenticated users on policy_financial_movements'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'invoice_financial_movements' AND policyname = 'Enable all access for authenticated users on invoice_financial_movements'
    ) THEN
...
-- 6. Atomic Endorsement Invoicing Transaction RPC
...
BEGIN
  ...
  -- 2. Update endorsement items computed premiums
  IF p_items_to_update IS NOT NULL AND jsonb_array_length(p_items_to_update) > 0 THEN
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_items_to_update) AS x(id uuid, premium numeric)
    LOOP
      UPDATE public.endorsement_items
      SET premium = v_item.premium
      WHERE id = v_item.id;
    END LOOP;
  END IF;
