-- Migration: secure_financial_movements
-- Date: 2026-08-30 00:00:00

-- 1. Drop wide-open RLS policies on financial movements
DROP POLICY IF EXISTS "Enable all access for authenticated users on policy_financial_movements" ON public.policy_financial_movements;
DROP POLICY IF EXISTS "Enable all access for authenticated users on invoice_financial_movements" ON public.invoice_financial_movements;

-- 2. Create secure scoped RLS policies on policy_financial_movements
CREATE POLICY "Users can see financial movements of allowed policies"
ON public.policy_financial_movements FOR SELECT TO authenticated
USING (
  public.is_admin_or_manager() OR policy_id IN (SELECT id FROM public.policies)
);

CREATE POLICY "Users can manage financial movements of allowed policies"
ON public.policy_financial_movements FOR ALL TO authenticated
USING (
  public.is_admin_or_manager() OR policy_id IN (
    SELECT id FROM public.policies WHERE client_company_id IN (
      SELECT id FROM public.companies WHERE assigned_user_id = auth.uid()::text
    ) OR client_company_id = public.get_auth_user_company_id()
  )
);

-- 3. Create secure scoped RLS policies on invoice_financial_movements
CREATE POLICY "Users can see invoice financial movements of allowed policies"
ON public.invoice_financial_movements FOR SELECT TO authenticated
USING (
  public.is_admin_or_manager() OR invoice_id IN (SELECT id FROM public.invoices)
);

CREATE POLICY "Users can manage invoice financial movements of allowed policies"
ON public.invoice_financial_movements FOR ALL TO authenticated
USING (
  public.is_admin_or_manager() OR invoice_id IN (
    SELECT id FROM public.invoices WHERE client_company_id IN (
      SELECT id FROM public.companies WHERE assigned_user_id = auth.uid()::text
    ) OR client_company_id = public.get_auth_user_company_id()
  )
);

-- 4. Drop the old process_endorsement_invoicing function (14 parameters)
DROP FUNCTION IF EXISTS public.process_endorsement_invoicing(
  uuid, text, text, date, date, numeric, text, numeric, numeric, jsonb, jsonb, jsonb, jsonb, uuid
);

-- 5. Define new process_endorsement_invoicing function (15 parameters, supports p_lob_key)
