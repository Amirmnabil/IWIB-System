...
import {
  ...
  Copy, ExternalLink, GitBranch, Paperclip, Upload, X, Calendar, Download, FolderOpen
} from "lucide-react";
...
// ─── Status config ─────────────────────────────────────────────────────────────
const STATUS_CONFIG: Record<UnderwritingStatus, { key: string; label: string; color: string; dotColor: string }> = {
  pending: {
    key: "status_pending",
    label: "Pending",
...
function CollapsibleSection({
  ...
  return (
    <Card className="rounded-2xl border-border shadow-sm overflow-hidden">
      <button
        onClick={() => setOpen((o) => !o)}
        className={cn("w-full flex items-center justify-between p-5 hover:bg-muted/30 transition-colors", isRtl ? "text-right" : "text-left")}
      >
...
function ClientDocumentsSection({
  ...
  docs: { name: string; url: string; uploaded_at: string }[];
}) {
    ...
    <Card className="rounded-2xl border-border shadow-sm overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-foreground">{t('clientDocuments' as any) || "Client Documents"}</span>
          <Badge
            variant="outline"
            className="text-[10px] font-black bg-emerald-50 border-emerald-200 text-emerald-700"
          >
            {docs.length} {t('filesUploaded' as any) || "files uploaded"}
          </Badge>
        </div>
        {open ? (
          <ChevronUp className="w-4 h-4 text-muted-foreground" />
            ...
            {docs.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 gap-3">
                ...
                <p className="text-xs text-muted-foreground/70 text-center max-w-xs">
                  {t('clientDocsUploadHint' as any) || "Documents are uploaded when the sales agent triggers Request Quotation on the Companies page."}
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {docs.map((doc, idx) => (
                  <a
                    ...
                    className="group flex items-start gap-3 p-3.5 rounded-xl border border-border bg-card hover:border-emerald-300 hover:bg-emerald-50/40 transition-all duration-200 shadow-sm"
                  >
                    <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-emerald-100 to-teal-50 flex items-center justify-center shrink-0 border border-emerald-200 shadow-sm group-hover:from-emerald-200 group-hover:to-teal-100 transition-all">
                      <FileText className="w-4 h-4 text-emerald-700" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p
                        className="text-sm font-bold text-foreground truncate group-hover:text-emerald-800 transition-colors"
                        title={doc.name}
                      >
                        {doc.name}
                      </p>
                      <p className="text-[11px] text-muted-foreground mt-0.5 font-medium">
                        {t('uploaded' as any) || "Uploaded"}{" "}
                        {doc.uploaded_at
                          ? format(new Date(doc.uploaded_at), lang === 'ar' ? 'dd-MM-yyyy' : "MMM d, yyyy")
                          : "—"}
                      </p>
                    </div>
                    <Download className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </a>
                ))}
              </div>
            )}
    ...
    <tr className="border-b border-border/60 hover:bg-muted/20 transition-colors group">
      ...
      <td className="px-4 py-3 text-xs text-muted-foreground">
        <div className="flex items-center gap-1.5">
          ...
          {offer.received_date ? format(new Date(offer.received_date), lang === 'ar' ? 'dd-MM-yyyy' : "MMM d, yyyy") : "—"}
        </div>
      </td>
      <td className="px-4 py-3">
        {offer.file_url ? (
          <a
            ...
            title={t('viewOfferPdf' as any) || "View Offer PDF"}
          >
          ...
          <span className="text-xs text-muted-foreground italic">{t('noDocument' as any) || "No document"}</span>
        )}
      </td>
      ...
      <td className="px-4 py-3 text-right">
        <button
          ...
          title={t('deleteOffer' as any) || "Delete Offer"}
...
export default function QuotationDetailsPage() {
  ...
  const loadProspect = useCallback(async () => {
    ...
    try {
      ...
      toast({ variant: "destructive", title: t('failedLoadQuotation' as any) || "Failed to load quotation", description: err?.message });
    } finally {
  ...
  const addOffer = async () => {
    ...
    if (offerFile) {
      ...
      try {
        ...
        toast({ variant: "destructive", title: t('pdfUploadFailed' as any) || "PDF upload failed", description: err?.message });
  ...
  const addVersion = () => {
    ...
    toast({
      title: `${t('created' as any) || "Created"} ${newV.label}`,
      description: prevLabel
        ? `${t('prefilledVersionMsg' as any) || "Pre-filled with offers from"} ${prevLabel}.`
        : t('newQuotationVersionReady' as any) || "New quotation version ready.",
    });
  };
  ...
  const handleSave = () => {
    ...
    UnderwritingService.saveUnderwritingVersions(id, companyId, versions, uwStatus)
      .then(() => {
        ...
        toast({
          title: t('syncedWithProspects' as any) || "✅ Synced with Prospects",
          description: t('syncedWithProspectsDesc' as any) || "All offers and PDFs are now visible in Prospects → Pricing Options.",
        });
      })
      .catch((err: any) => {
        toast({ variant: "destructive", title: t('syncFailed' as any) || "Sync failed", description: err?.message });
      });
  };
  ...
  const handleStatusChange = async (status: UnderwritingStatus) => {
    ...
    try {
      ...
      const statusTransKey = STATUS_CONFIG[status].key;
      toast({ title: `${t('statusUpdated' as any) || "Status updated to"} "${t(statusTransKey as any) || statusLabel}"` });
    } catch (err: any) {
      toast({ variant: "destructive", title: t('statusUpdateFailed' as any) || "Status update failed", description: err?.message });
    }
  };
  ...
  if (loadingProspect) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-muted-foreground font-semibold">{t('loadingQuotation' as any) || "Loading quotation…"}</p>
      </div>
    );
  }
  ...
  if (!prospect) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-foreground font-bold text-lg">{t('quotationNotFound' as any) || "Quotation not found"}</p>
        <Button onClick={() => router.back()} variant="outline">
          <ArrowLeft className={cn("w-4 h-4", isRtl ? "ml-2 rotate-180" : "mr-2")} /> {t('goBack' as any) || "Go Back"}
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-5xl mx-auto pb-24 space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground font-medium">
              {t('quotationRequest' as any) || "Quotation request"} • {format(new Date(prospect.created_at), lang === 'ar' ? 'dd-MM-yyyy' : "MMMM d, yyyy")}
            </p>
          </div>
        </div>
      ...
      ═══════════════════════════════════════════════════════════════════ */}
      <CollapsibleSection
        title={t('crmProspectData' as any) || "CRM & Prospect Data"}
        icon={Info}
        defaultOpen={false}
        badge={t('clickToExpand' as any) || "Click to expand"}
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <DataField label={t('company' as any) || "Company"} value={prospect.company_name} />
          <DataField label={t('pipelineStage' as any) || "Pipeline Stage"} value={prospect.pipeline_stage?.replace(/_/g, " ")} />
          <DataField label={t('assignedTo' as any) || "Assigned To"} value={prospect.assigned_user_name || "—"} />
          <DataField
            label={t('currentInsurer' as any) || "Current Insurer"}
            value={details?.insurance_company || prospect.current_insurer || "—"}
          />
          <DataField label={t('currentTPA' as any) || "Current TPA"} value={prospect.current_tpa || "—"} />
          <DataField
            label={t('expectedClose' as any) || "Expected Close"}
            value={
              prospect.expected_close_date
                ? format(new Date(prospect.expected_close_date), lang === 'ar' ? 'dd-MM-yyyy' : "MMM d, yyyy")
                : "—"
            }
          />
          <DataField
            label={t('estimatedValue' as any) || "Estimated Value"}
            value={
          ...
          />
          <DataField label={t('probability' as any) || "Probability"} value={prospect.probability ? `${prospect.probability}%` : "—"} />
          <DataField label={t('requestedProducts' as any) || "Requested Products"} value={(prospect.requested_products || []).join(", ") || "—"} />
        </div>
        {prospect.notes && (
          <div className="mt-4 p-3 bg-muted/50 rounded-xl">
            <p className="text-[11px] font-black text-muted-foreground uppercase tracking-wider mb-1">{t('notes' as any) || "Notes"}</p>
        ...
        <div className="mt-4 pt-3 border-t border-border">
          ...
          >
            ...
            {t('viewFullCompanyProfile' as any) || "View Full Company Profile"}
          </button>
        </div>
      </CollapsibleSection>

      {/* ═══════════════════════════════════════════════════════════════════
          SECTION B — Client Documents (uploaded during Request Quotation)
      ═══════════════════════════════════════════════════════════════════ */}
      <ClientDocumentsSection
        docs={
          (details?.client_documents as { name: string; url: string; uploaded_at: string }[]) ||
          []
        }
      />

      {/* ═══════════════════════════════════════════════════════════════════
          SECTION C — Underwriting Offers (Main)
      ═══════════════════════════════════════════════════════════════════ */}
      <Card className="rounded-2xl border-border shadow-sm overflow-hidden">
        <CardHeader className="border-b border-border px-5 py-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-base font-black text-foreground">
                  {t('insurerOffers' as any) || "Insurer Offers"}
                </CardTitle>
                <p className="text-xs text-muted-foreground font-medium mt-0.5">
                  {t('insurerOffersDesc' as any) || "Upload and record offers received from insurance companies"}
                </p>
              </div>
            </div>
            <Button
            ...
            >
              ...
              {t('newVersion' as any) || "New Version"}
            </Button>
          </div>
        </CardHeader>
        ...
        <CardContent className="p-5 space-y-5">
            ...
            <>
              <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                <div className="flex items-center gap-2">
                  ...
                  {t('created' as any) || "Created"} {format(new Date(activeVersion.created_at), lang === 'ar' ? 'dd-MM-yyyy HH:mm' : "MMM d, yyyy 'at' HH:mm")}
                </div>
                {activeVersionIdx > 0 && (
                  <span className="text-[11px] font-semibold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">
                    {t('prefilledVersionMsg' as any) || "Pre-filled with offers from"} Version {activeVersionIdx}
                  </span>
                )}
              </div>
                    ...
                    {activeVersion.offers.length === 0 ? (
                      <tr>
                        <td colSpan={7} className="px-4 py-8 text-center text-sm text-slate-400 italic">
                          {t('noOffersAddedYet' as any) || "No offers added yet — use the form below to record insurer responses."}
                        </td>
                      </tr>
                    ) : (
              ...
              <div className="border border-dashed border-indigo-200 rounded-xl p-4 bg-indigo-50/30 space-y-4">
                <p className="text-[11px] font-black text-indigo-800 uppercase tracking-widest flex items-center gap-1.5">
                  ...
                  {t('addInsurerOfferTo' as any) || "Add Insurer Offer to"} {activeVersion.label}
                </p>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={offerInsurer} onValueChange={setOfferInsurer}>
                      <SelectTrigger className="bg-white h-9 text-sm">
                        <SelectValue placeholder={t('selectInsurerPlaceholder' as any) || "Select insurer…"} />
                      </SelectTrigger>
                      <SelectContent>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                  ...
                  <div className="space-y-1.5 sm:col-span-2">
                    <Label className="text-[11px] font-black text-muted-foreground uppercase tracking-wider">
                      {t('uploadOfferPdfDoc' as any) || "Upload Offer PDF / Document"}
                    </Label>
                    <div className="flex items-center gap-2">
                      <label className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-dashed border-indigo-300 bg-white hover:bg-indigo-50 cursor-pointer text-xs font-bold text-indigo-700 transition-colors shrink-0">
                        ...
                        {t('chooseFile' as any) || "Choose File"}
                        <input
                          type="file"
                      ...
                      {offerFile ? (
                        ...
                        <span className="text-xs text-muted-foreground italic">{t('noFileSelected' as any) || "No file selected (optional)"}</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {uploadingFile ? (t('uploadingPdf' as any) || "Uploading PDF…") : (t('addOffer' as any) || "Add Offer")}
                    </Button>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-between pt-2 border-t border-border">
            ...
            <div className="flex items-center gap-2">
              {lastSynced ? (
                <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-emerald-600">
                  ...
                  {t('synced' as any) || "Synced"} {format(lastSynced, "HH:mm")}
                </span>
              ) : (
                <span className="text-xs text-muted-foreground font-medium">
                  {t('savesInstantly' as any) || "Saves instantly in background"}
                </span>
              )}
            </div>
            <Button
            ...
            >
              ...
              {syncPulse ? (t('syncing' as any) || "Syncing…") : (t('saveSync' as any) || "Save & Sync")}
            </Button>
          </div>
        </CardContent>
      </Card>
