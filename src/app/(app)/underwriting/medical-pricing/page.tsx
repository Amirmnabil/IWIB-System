...
export default function SMEMedicalPricingTool() {
  ...
  const [downloadLanguageSelectOpen, setDownloadLanguageSelectOpen] = useState(false);
  const [selectedDownloadLang, setSelectedDownloadLang] = useState<'en' | 'ar' | 'both'>('both');
  const [quoteToDownload, setQuoteToDownload] = useState<SMEOffer | null>(null);
  const [downloadingQuoteLanguage, setDownloadingQuoteLanguage] = useState<'en' | 'ar' | 'both' | null>(null);
  const [showOnlyActive, setShowOnlyActive] = useState(false);
  const [filters, setFilters] = useState<PlanFilters>(INITIAL_FILTERS);
  const [isFilterSidebarOpen, setIsFilterSidebarOpen] = useState(false);
  const pdfContainerRefEn = useRef<HTMLDivElement>(null);
  const pdfContainerRefAr = useRef<HTMLDivElement>(null);
  ...
  const handleSaveQuotation = async () => {
    ...
    try {
      ...
      const generatePdf = async (lang: 'en' | 'ar', ref: React.RefObject<HTMLDivElement | null>) => {
        ...
        const slideCount = ref.current.children.length;
        for (let i = 0; i < slideCount; i++) {
          const slide = ref.current.children[i] as HTMLElement;
        ...
        pdf.save(`${offerName || 'SME_Offer'}_${lang.toUpperCase()}.pdf`);
      ...
      try {
        ...
        let suffix = '';
        if (selectedDownloadLang === 'en' || selectedDownloadLang === 'both') {
          pdfBlob = await generatePdf('en', pdfContainerRefEn);
          suffix = 'EN';
        }
        if (selectedDownloadLang === 'ar' || selectedDownloadLang === 'both') {
          pdfBlob = await generatePdf('ar', pdfContainerRefAr);
          suffix = 'AR';
        }

        if (pdfBlob) {
          const fileName = `offers/${offerId}_${Date.now()}_${suffix}.pdf`;
  ...
  useEffect(() => {
    if (downloadingQuote && downloadingQuoteLanguage && pdfContainerRefEn.current && pdfContainerRefAr.current) {
      const triggerDownload = async () => {
        try {
          ...
          const generatePdf = async (lang: 'en' | 'ar', ref: React.RefObject<HTMLDivElement | null>) => {
            ...
            pdf.save(`${downloadingQuote.offer_name || 'SME_Offer'}_${lang.toUpperCase()}.pdf`);
          };

          if (downloadingQuoteLanguage === 'en' || downloadingQuoteLanguage === 'both') {
            await generatePdf('en', pdfContainerRefEn);
          }
          if (downloadingQuoteLanguage === 'ar' || downloadingQuoteLanguage === 'both') {
            await generatePdf('ar', pdfContainerRefAr);
          }

          toast({ title: "Offer Downloaded" });
        } catch (err: any) {
          console.error("PDF generation failed. Message:", err?.message, "Stack:", err?.stack, "Raw:", err);
          toast({ variant: 'destructive', title: "Download Failed" });
        } finally {
          setDownloadingQuote(null);
          setDownloadingQuoteLanguage(null);
        }
      };
      setTimeout(triggerDownload, 500);
    }
  }, [downloadingQuote, downloadingQuoteLanguage]);
                    ...
                    {isLoadingQuotations ? (
                      ...
                      <TableRow key={quote.id} className="hover:bg-background transition-colors group">
                        ...
                        <TableCell className="text-right pr-6">
                          <Button variant="ghost" size="sm" onClick={() => { setQuoteToDownload(quote); setDownloadLanguageSelectOpen(true); }} disabled={downloadingQuote?.id === quote.id}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {!isViewMode && (
                  <Button variant="default" className="rounded-full h-10 px-6 font-bold bg-primary hover:bg-indigo-700 shadow-md" onClick={() => {
                    ...
                    setGeneratedOfferCode(`SME-${new Date().getFullYear()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`);
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[calc(100vh-120px)] gap-6 -m-4 lg:-m-6 bg-background p-4 lg:p-6">
      ...
      <div style={{ position: 'absolute', top: -9999, left: -9999, pointerEvents: 'none', zIndex: -9999 }}>
        {(downloadingQuote || isOfferDialogOpen) && (
          <>
            <OfferPDFTemplate
              ref={pdfContainerRefEn}
              language="en"
              offerName={downloadingQuote ? downloadingQuote.offer_name : offerName}
              companyName={downloadingQuote ? downloadingQuote.company_name : companyInfo.name}
              date={format(new Date(downloadingQuote ? downloadingQuote.created_at : new Date()), 'dd/MM/yyyy')}
              plans={ALL_PLANS.filter(p => (downloadingQuote ? downloadingQuote.selected_plans.planIds : selectedPlanIds).includes(p.id))}
              snapshots={downloadingQuote ? downloadingQuote.selected_plans.snapshots : generateSnapshots()}
              cashbackAmount={downloadingQuote ? downloadingQuote.selected_plans.cashbackAmount : (cashbackAmount ? Number(cashbackAmount) : undefined)}
              offerCode={downloadingQuote ? (downloadingQuote.selected_plans as any).offerCode : generatedOfferCode}
              memberCounts={
                downloadingQuote ? {
                  employee: downloadingQuote.selected_plans.members?.filter((m: any) => m.type === 'Employee' && m.isValid).length || 0,
                  spouse: downloadingQuote.selected_plans.members?.filter((m: any) => m.type === 'Spouse' && m.isValid).length || 0,
                  child: downloadingQuote.selected_plans.members?.filter((m: any) => m.type === 'Child' && m.isValid).length || 0,
                } : {
                  employee: members.filter(m => m.type === 'Employee' && m.isValid).length,
                  spouse: members.filter(m => m.type === 'Spouse' && m.isValid).length,
                  child: members.filter(m => m.type === 'Child' && m.isValid).length,
                }
              }
            />
            <OfferPDFTemplate
              ref={pdfContainerRefAr}
              language="ar"
              offerName={downloadingQuote ? downloadingQuote.offer_name : offerName}
              companyName={downloadingQuote ? downloadingQuote.company_name : companyInfo.name}
              date={format(new Date(downloadingQuote ? downloadingQuote.created_at : new Date()), 'dd/MM/yyyy')}
      ...
      <Dialog open={isOfferDialogOpen} onOpenChange={setIsOfferDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              <Label className="font-bold">Document Language</Label>
              <div className="grid grid-cols-3 gap-2">
                <Button type="button" variant={selectedDownloadLang === 'en' ? 'default' : 'outline'} onClick={() => setSelectedDownloadLang('en')}>English</Button>
                <Button type="button" variant={selectedDownloadLang === 'ar' ? 'default' : 'outline'} onClick={() => setSelectedDownloadLang('ar')}>Arabic (العربية)</Button>
                <Button type="button" variant={selectedDownloadLang === 'both' ? 'default' : 'outline'} onClick={() => setSelectedDownloadLang('both')}>Both</Button>
              </div>
            </div>
            <div className="space-y-2">
      ...
      <Dialog open={downloadLanguageSelectOpen} onOpenChange={setDownloadLanguageSelectOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Select Document Language</DialogTitle>
            <DialogDescription>Please select your preferred language for the offer:</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-3 gap-4 py-4">
            <Button variant={selectedDownloadLang === 'en' ? 'default' : 'outline'} onClick={() => setSelectedDownloadLang('en')}>English</Button>
            <Button variant={selectedDownloadLang === 'ar' ? 'default' : 'outline'} onClick={() => setSelectedDownloadLang('ar')}>Arabic (العربية)</Button>
            <Button variant={selectedDownloadLang === 'both' ? 'default' : 'outline'} onClick={() => setSelectedDownloadLang('both')}>Both</Button>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDownloadLanguageSelectOpen(false)}>Cancel</Button>
            <Button onClick={() => {
              setDownloadingQuoteLanguage(selectedDownloadLang);
              setDownloadingQuote(quoteToDownload);
              setDownloadLanguageSelectOpen(false);
            }}>Proceed</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
