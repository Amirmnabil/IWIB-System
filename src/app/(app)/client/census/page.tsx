...
const LOCAL_TRANSLATIONS: Record<string, Record<string, string>> = {
  en: {
    ...
    activeInsuredDesc: "Listed below are all currently covered personnel active on this policy contract.",
    pendingRequests: "Pending Extension Requests",
    pendingRequestsDesc: "Track additions and cancellations requests currently being reviewed by your insurance brokers.",
    noActiveMembers: "No active census members match your filter query.",
    noPendingRequests: "No pending addition or deletion extension requests registered.",
    name: "Name",
    relation: "Relation",
    ...
    status: "Status",
    additionRequest: "Addition Request",
    ...
    additionsDesc: "Add new employees or dependents to the contract census document. This generates a pending Addition Endorsement.",
  ...
  ar: {
    censusPortal: "بوابة بيانات المؤمن عليهم",
    activeContract: "العقد النشط",
    insurer: "شركة التأمين",
    validity: "صلاحية العقد",
    startInsurance: "بداية التأمين",
    additions: "الإضافات",
    deletions: "الحذوفات",
    currentActive: "الأعضاء الحاليين",
    members: "أعضاء",
    requests: "طلبات",
    searchPlaceholder: "البحث بالاسم أو الرقم القومي...",
    downloadCensus: "تحميل قائمة المؤمن عليهم",
    downloadAdditions: "تحميل قائمة الإضافات",
    downloadDeletions: "تحميل قائمة الحذوفات",
    activeInsuredMembers: "أعضاء التأمين الحاليين",
    activeInsuredDesc: "القائمة أدناه توضح جميع الموظفين النشطين والمغطين حالياً بموجب هذا العقد.",
    pendingRequests: "طلبات الإضافة والحذف المعلقة",
    pendingRequestsDesc: "متابعة طلبات الإضافة والإلغاء قيد المراجعة حالياً من قبل وسيط التأمين.",
    noActiveMembers: "لا يوجد أعضاء مطابقين للبحث.",
    noPendingRequests: "لا توجد طلبات إضافة أو حذف معلقة مسجلة.",
    name: "الاسم",
    ...
    status: "الحالة",
    additionRequest: "طلب إضافة",
    cancellationRequest: "طلب إلغاء",
    pendingReview: "قيد المراجعة",
    undoDeletion: "تراجع عن الحذف",
    unassociatedAccount: "حساب غير مرتبط بعقد",
    unassociatedDesc: "حسابك غير مرتبط بأي عقد تأمين نشط. يرجى التواصل مع الدعم لتفعيل البوابة الخاصة بك.",
    loading: "جاري تحميل بيانات البوابة...",
    cancel: "إلغاء",
    confirmRequest: "تأكيد الطلب",
    requestMemberAdditions: "طلب إضافة أعضاء",
    additionsDesc: "إضافة موظفين جدد أو تابعين إلى قائمة التأمين. سيقوم هذا بإنشاء ملحق إضافة معلق.",
    ...
    linkedMain: "العضو الرئيسي المرتبط (الموظف) *",
    nationality: "الجنسية",
    location: "الموقع",
    jobTitle: "المسمى الوظيفي",
    staffCode: "كود الموظف",
    submitRequest: "إرسال الطلب",
    uploadExcel: "تحميل جدول إكسل",
    excelDesc: "اسحب وأسقط ملف إكسل هنا، أو انقر للتصفح. يدعم صيغ (.xlsx, .xls).",
    downloadTemplate: "تحميل النموذج الفارغ",
    chooseFile: "اختيار ملف",
    reversalConfirm: "تحذير: سيتم تطبيق الحذف بعد 48 ساعة. يمكنك التراجع عن الحذف خلال هذه المدة، ولكن بعد مرور 48 ساعة لا يمكن إلغاء الطلب.",
    reversalTitle: "طلب إلغاء عضوية مؤمن عليه",
    reversingMultiple: "أنت تقوم بطلب إلغاء العضوية لـ {count} أعضاء:",
    age: "العمر",
    yrs: "سنة",
    employee: "موظف",
    spouse: "زوج / زوجة",
...
export default function ClientCensusPage() {
  ...
  const tr = (key: keyof typeof LOCAL_TRANSLATIONS['en']) => {
    return LOCAL_TRANSLATIONS[lang === 'ar' ? 'ar' : 'en'][key] || key;
  };
  ...
  const censusMetrics = useMemo(() => {
    ...
    const startCount = activeMembers.filter((m: any) => !m.addition_date || m.addition_date === activePolicy?.start_date).length;
    const additionsCount = activeMembers.filter((m: any) => m.addition_date && m.addition_date !== activePolicy?.start_date).length + pendingAdditionsCount;
    const deletionsCount = activeMembers.filter((m: any) => m.deletion_date).length + pendingDeletionsCount;
  ...
  const handleDownloadTemplate = () => {
    const templateHeaders = [
      "Member Name", "Staff Code", "Date Of Birth", "Gender", "Relation", 
      "Nationality", "National ID", "Plan Category", "Location", 
      "Department", "Job Title", "Mobile Number"
    ];
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const payload = json.map((row: any, index: number) => {
          ...
          if (!valResult.isValid) {
            const errList = Object.entries(valResult.errors).map(([field, msg]) => `${field}: ${msg}`).join("; ");
            throw new Error(`Row ${index + 2} (${memberObj.member_name || 'Unnamed'}): ${errList}`);
          }
          ...
          return {
            ...
            details: {
              ...
              date_of_birth: memberObj.date_of_birth,
              gender: memberObj.gender,
              relation: memberObj.relation,
              nationality: row["Nationality"] || "Egyptian",
              plan_category: memberObj.plan_category,
              location: row["Location"] || "",
              department: row["Department"] || "",
              job_title: row["Job Title"] || "",
              mobile_number: memberObj.mobile_number,
              linked_main_member_id: memberObj.linked_main_member_id || null,
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="border border-border/70 shadow-sm bg-gradient-to-br from-indigo-500/5 to-indigo-500/10 dark:from-indigo-950/10 dark:to-indigo-900/5">
          <CardContent className="p-5 flex flex-col justify-between h-full">
            ...
            <p className="text-[10px] text-muted-foreground mt-1">Initial census count at contract launch</p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border/70 shadow-sm bg-gradient-to-br from-emerald-500/5 to-emerald-500/10 dark:from-emerald-950/10 dark:to-emerald-900/5">
          <CardContent className="p-5 flex flex-col justify-between h-full">
            ...
            <p className="text-[10px] text-muted-foreground mt-1">New additions processed during contract term</p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border/70 shadow-sm bg-gradient-to-br from-rose-500/5 to-rose-500/10 dark:from-rose-950/10 dark:to-rose-900/5">
          <CardContent className="p-5 flex flex-col justify-between h-full">
            ...
            <p className="text-[10px] text-muted-foreground mt-1">Coverage terminations processed during term</p>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border/70 shadow-sm bg-gradient-to-br from-indigo-500/10 via-purple-500/5 to-indigo-500/20 dark:from-indigo-950/20 dark:via-purple-950/10 dark:to-background">
          <CardContent className="p-5 flex flex-col justify-between h-full">
            <span className="text-muted-foreground text-xs font-semibold uppercase tracking-wider">Current Active</span>
            <div className="flex items-baseline gap-2 mt-2">
              <span className="text-3xl font-black text-primary">{censusMetrics.currentActive}</span>
              <span className="text-xs text-muted-foreground">Members</span>
            </div>
            <p className="text-[10px] text-muted-foreground mt-1">Currently covered active personnel</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border border-border/85 shadow-sm overflow-hidden bg-card">
        ...
        >
          ...
          <div className="flex items-center gap-3" onClick={e => e.stopPropagation()}>
            ...
            <Button size="sm" variant="outline" onClick={handleDownloadCensus} className="h-9 text-xs font-bold gap-1.5 bg-background shadow-sm border-border">
              <Download className="w-3.5 h-3.5" /> {tr('downloadCensus')}
            </Button>
          </div>
        </div>
            ...
            <TabsContent value="manual" className="mt-4">
              <form onSubmit={handleManualAddSubmit} className="space-y-4">
                <div className="max-h-[50vh] overflow-y-auto pr-2 pb-2 space-y-4">
                  ...
                  {/* Linked Main Member ID (Visible only if relation is not Employee) */}
          ...
          {selectedMember ? (
            <div className="bg-slate-50 dark:bg-slate-900/30 p-4 border rounded-xl space-y-2 mt-4 text-xs md:text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground font-medium">{tr('name')}:</span>
                <span className="font-bold text-foreground">{selectedMember.member_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground font-medium">{tr('nationalId')}:</span>
              ...
              <div className="flex justify-between">
                <span className="text-muted-foreground font-medium">{tr('relation')}:</span>
                <span className="font-semibold text-foreground">{translateRelation(selectedMember.relation)}</span>
              </div>
            </div>
          ) : selectedMemberIds.length > 0 ? (
