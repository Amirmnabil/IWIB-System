...
export default function CreateEndorsementWizard({ policy: initialPolicy, insurer: initialInsurer, onClose, onSuccess }: CreateEndorsementWizardProps) {
  ...
  const handleParentStaffCodeSearch = (query: string) => {
    setParentStaffCode(query);
                        ...
                        <div className="px-4 pb-4 pt-1 animate-in fade-in duration-150">
                          ...
                          {isDependent && (
                            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg space-y-2 mb-3">
                              <Label className="text-[11px] font-bold text-blue-800">Search Parent Employee (Name, National ID, or Staff ID) *</Label>
                              <Input value={parentStaffCode} onChange={e => handleParentStaffCodeSearch(e.target.value)} placeholder="Search by name, National ID, or Staff ID..." className="h-9 rounded-lg text-sm" />
                              {parentSearchResult && <div className="flex items-center gap-2 p-2 bg-emerald-50 border border-emerald-200 rounded-lg text-[11px] text-emerald-700 font-semibold"><CheckCircle2 className="w-3.5 h-3.5" /> Linked to: {parentSearchResult.member_name} (Staff ID: {parentSearchResult.staff_code || "N/A"})</div>}
