  ...
  if (activities.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200">
        ...
        <h3 className="text-lg font-semibold text-slate-800">{t('noActivities')}</h3>
        ...
        <Button onClick={onAddActivity} className="bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-200 gap-2">
        ...
        {sortedActivities.map((activity, index) => {
          ...
          return (
            ...
            >
              ...
              {/* Content Card */}
              <Card className="flex-1 rounded-2xl border-slate-100 shadow-sm overflow-hidden group-hover:shadow-md group-hover:border-slate-200 transition-all duration-300">
                  ...
                  <div className="p-4 sm:p-5">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-40">
                            ...
                            <DropdownMenuItem onClick={() => onDeleteActivity?.(activity.id)} className="gap-2 text-red-600 focus:text-red-600">
                    ...
                    {(activity.assigned_to_name || activity.result) && (
                      <div className="mt-4 pt-4 border-t border-slate-50 flex flex-wrap items-center gap-4">
                        ...
                        {activity.status && (
                          <Badge variant="secondary" className="text-[10px] bg-slate-100 text-slate-600 hover:bg-slate-200 border-none px-2">
                            {activity.status}
                          </Badge>
                        )}
                      </div>
